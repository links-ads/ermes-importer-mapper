        ...
        try:
            ...
            if message_schema.type in ["delete", "update"]:
                with db_session() as session:
                    resources = data_storage_manager.get_resources(session=session, resource_id=message_schema.id)
                    LOG.info(resources)
            ...
            if message_schema.type in ["create", "update"]:
                data_list = data_retrieval_manager.download_data(message_schema=message_schema)
                ...
                delete_oldest_resources(datatypes=impacted_datatypes)
        except Exception as e:
            LOG.error(e, exc_info=True)

    def delete_oldest_resources(datatypes: List[str] = None):
        with db_session() as session:
            expired_resources = []
            for dtype in datatypes:
                ...
                resources = data_storage_manager.get_resources(
                    session=session, datatype_ids=[dtype], expire_on=datetime.utcnow()
                )
                ...
                if delete_after_count and delete_after_count > 0:
                    resources = data_storage_manager.get_resources(
                        session=session, datatype_ids=[dtype], order_by=GeoserverResource.created_at
                    )
